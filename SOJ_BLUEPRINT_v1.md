...
**F6 — Expansão em ondas (aprovada em 2026-07-06; UMA onda por sessão, auditoria do advogado entre ondas; tudo testado antes no TESTE_FICTICIO):**
- **Onda 1 — Fundação de dados (ENTREGUE em 2026-07-06):** CASO.yaml ganha `fase_processual` (pre_protocolo/postulatoria/instrutoria/decisoria/recursal/cumprimento/encerrado), bloco `financeiro` (contrato fixo|percentual|exito|misto|probono + valor/parcelas/doc_assinado; custas[]; recebimentos[]) e `audiencias[]`; decisões judiciais recebidas pela porta única ganham juízo/comarca/resultado; **contrato de honorários assinado = pendência padrão do intake** (salvo probono/gratuidade marcada) — *sem bloquear o protocolo*: retificação do advogado na auditoria da Onda 1 (regularizável em qualquer fase do processo); views: FINANCEIRO.md por caso + agregado financeiro e "verbetes a vencer" no PAINEL; comando **"revalidar biblioteca"** (verbetes vencidos em lote, com lista dos casos ativos afetados via "casos que citam") com lembrete automático na 1ª sessão do mês.
- **Onda 2 — Motores (ENTREGUE em 2026-07-06):** gatilhos de fase no roteador — rotas `contestacao` (→ propõe réplica com prazo; fase→postulatória), `sentenca` (→ propõe embargos/apelação; recurso = Tier B; fase→decisória), `transito` (→ propõe cumprimento; fase→cumprimento) e `ata_audiencia` (audiência realizada + ata vinculada + colheita; fase→instrutória), todas com a regra dura do prazo-antes-de-tudo e atualização automática da `fase_processual`; peça intermediária sem template nasce do zero e o texto aprovado vira candidato a template na colheita (`COLHEITA:`); "guardar modelo → extrair template" para peças antigas do advogado (proposta de template ratificável); comando "preparar audiência" (`preparar_audiencia.py`: pasta AUDIENCIAS/ com roteiro para revisão do advogado — perguntas por testemunha, ataques ao depoimento adverso, provas DOC-refs, riscos, logística — + PZ no vigia/Calendar; ata volta pela porta única); `relatorio_mensal.py` (ESCRITORIO/RELATORIOS/AAAA-MM.md — inclui reprovações de gate E MOTIVOS; horas e custo de IA como ESTIMATIVA marcada).
- **Onda 3 — Conhecimento (ENTREGUE em 2026-07-06):** Fase 5 das skills concluída — `bancario_ccb` (defesa em execução de CCB × cooperativas, **modo defesa nativo**: caso da área nasce em polo passivo) e `consumidor_aguas` (negativação × Águas do Pará, JEC) adaptadas ao contrato da seção 9 com **100% do conteúdo validado preservado por cópia íntegra** (referencias/checklists/playbooks/templates/scripts + SKILL_original.md; integridade conferida byte a byte) — a camada de contrato (MODULO, praxe com Tier A/B, reservadas, checklist, teses COM antiteses, anti-erro) aponta para o conteúdo, não o reescreve; jurisprudência herdada vira verbete datado na BASE_LEGAL no primeiro uso em peça nova (G3 garante); esqueletos `trabalhista/` e `previdenciario/` criados (praxe e templates vazios de propósito — nascem do caso 1); camada de PRAXE LOCAL implantada nas árvores (anotação com contagem e fontes do Acervo; recomendação só com ≥3 amostras consistentes do mesmo juízo; local divergindo do nacional = sempre Tier B).
- **Onda 4 — Avaliação antes de construção:** transcrição local de atendimento (ex.: faster-whisper) — avaliar qualidade PT-BR/tempo/espaço e REPORTAR antes de instalar; desde já: áudio lacrado em 00_originais, consentimento do cliente como entrada obrigatória no DIARIO, transcrições prontas do celular aceitas como RELATO.
