...
## Blueprint de Arquitetura v1.10

**Data:** 2026-07-06
**Status:** v1.10 — **porta de importação** (seção 7): casos pré-trabalhados por colaborador entram por engenharia reversa com confiança zero + Relatório de Revisão do Colaborador; governança de autoria (quem fez/propôs/ratificou); classe de documentos instrumentais no roteador. v1.9: motores de autos e mídia. v1.8: Ondas 1–4. Histórico: v1.7 roteador+defesa · v1.6 Acervo · v1.5 mão dupla · v1.4 obra entregue · v1.3 D11 · v1.2 entrada · v1.1 D2.
...
**Porta de importação (trabalho de colaborador — v1.10):** quando um caso chega pré-trabalhado por sócio/estagiário (minuta pronta + provas + instrumentais), o sistema NÃO adota a peça: ele a **importa por engenharia reversa, com confiança zero** — fatos alegados na peça viram F## cruzados com as provas anexas (sem prova = *alegado* + pendência); pedidos viram PED##; toda citação de lei passa pela BASE_LEGAL; decisões embutidas (quantum, foro, tutelas) viram *propostas do colaborador* aguardando ratificação do titular. A peça entra como MINUTA_v01 (importada, etiquetada retroativamente) e percorre os gates normalmente. Saída imediata: **Relatório de Revisão do Colaborador** (juiz rigoroso + adversário + verificação de fontes sobre a peça dele). Governança de autoria: toda entrada e decisão registra a origem (quem fez, quem propôs, quem ratificou). Documentos **instrumentais** (procuração assinada, RG/CNH, contrato de honorários, declaração de hipossuficiência) formam classe própria no roteador — provam representação, não fato.
...
**Acervo de modelos (v1.6):** o escritório guarda peças e decisões de referência em `ESCRITORIO/ACERVO/` (IDs M-01…), cada uma com ficha de curadoria (origem, área, ano, *por que foi guardada*, ressalvas, resultado) e **anonimizada na entrada** — dados pessoais de terceiros não se armazenam, e peças de autos sigilosos não entram. Regra de ferro: **modelo é professor de estilo e técnica, nunca de lei** — citação jurídica vinda de modelo só entra em peça após verificação na BASE_LEGAL (modelo antigo = lei possivelmente superada). Modelos alimentam **o molde, não cada peça**: sua sabedoria é destilada para template/teses via colheita ratificada; consulta direta só sob demanda na revisão. Extensão natural: decisões e sentenças dos próprios processos entram no acervo — semente da calibração **local** da árvore (o que o juízo da comarca costuma deferir).
...
| **F5 — Expansão (em execução)** | Adaptar Águas do Pará e CCB ao kernel (módulos `consumidor_aguas` e `bancario_ccb`), preservando 100% do conteúdo validado | Onda 3 |
| **F6 — Ondas de melhoria (aprovadas em 06/07/2026)** | **Onda 1:** extensões da ficha (fase processual do ciclo de vida, bloco financeiro, audiências, juízo/comarca nas decisões) + views (financeiro no PAINEL, verbetes a vencer) + comando de revalidação da biblioteca. **Onda 2:** relatório mensal do escritório; preparação de audiência; gatilhos de fase no roteador (contestação → propõe réplica com prazo; sentença → propõe recurso). **Onda 3:** F5 + esqueletos trabalhista/previdenciário (praxe e template vazios de propósito — nascem do caso 1) + camada de praxe local na árvore (anotações com amostra mínima ≥3; divergência local = Tier B). **Onda 4 (escopo ampliado):** motor de mídia — transcrição LOCAL de áudios (atendimento, WhatsApp, audiências) após avaliação técnica reportada; export de conversa do WhatsApp como tipo do roteador (cronologia unificada com áudios degravados inline); áudio original lacrado com hash; degravação rotulada "de trabalho, não perícia". **Onda 5 — Motor de autos (ENTREGUE em 07/07/2026; OCR local = Windows.Media.Ocr pt-BR nativo do sistema, sem instalação; fato dos autos = campo `fonte_autos` com fls., aceito pelo G1):** comando "anexar autos": extração de texto por script (custo zero), OCR local do que não tiver camada de texto, ÍNDICE dos autos por documento/fls., **plano de leitura com orçamento aprovado ANTES do gasto** (pirâmide: peças e decisões integrais; certidões/ARs/guias nunca; resto sob demanda), destilação única para a ficha com referência por folha (F##/P## → fls.), cache de leitura (nada se relê). Todos os originais (autos e mídias) ganham hash SHA-256 na entrada. | 1 sessão por onda |
